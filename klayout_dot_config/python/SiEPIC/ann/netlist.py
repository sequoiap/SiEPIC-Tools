...
- SiEPIC.ann.models
    The ObjectModelNetlist requires this module in order to build Component models
- SiEPIC.extend
...
def _match_ports(net_id: str, component_list: list) -> list:
    ...
    a list of components provided by the caller (even if the component is connected
    to itself).

    Parameters
    ----------
    net_id : str
        The net id or name to which the components being searched for are connected.
    component_list : list
    ...
    [comp1, netidx1, comp2, netidx2]
        ...
        - comp1: Index of the first component in the list with a matching net id.
        - netidx1: Index of the net in the ordered net list of 'comp1' (corresponds
            to its column or row in the s-parameter matrix).
        - comp2: Index of the second component in the list with a matching net id.
        - netidx1: Index of the net in the ordered net list of 'comp2' (corresponds
            to its column or row in the s-parameter matrix).
    """
...
class ComponentSimulation:
    """
    This class is a simplified version of a Component in that it only contains an
    ordered list of nets, the frequency array, and the s-parameter matrix. It can
    be initialized with or without a Component model, allowing its attributes to be
    ...
    s : np.array
        A numpy ndarray of the s-parameter matrix for the given frequency range.
    """
    nets: list
    f: np.array
    s: np.array
...
def connect_circuit(netlist: ObjectModelNetlist) -> ComponentSimulation:
    ...
    -------
    ComponentSimulation
        After the circuit has been fully connected, the result is a single ComponentSimulation
        with fields f (frequency), s (s-matrix), and nets (external ports as strings of negative
        numbers).
    """
...
def strToSci(number) -> float:
    """
    Converts string representations of numbers written with abbreviated prefixes
    into a float with the proper exponent (e.g. '3u' -> 3e-6).
...
def get_sparameters(netlist: ObjectModelNetlist):
    """
    Gets the s-parameters matrix from a passed in ObjectModelNetlist by connecting
    all components.
